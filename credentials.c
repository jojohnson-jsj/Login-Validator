/*
  This program asks for a username and password. It then hashes the given password with SHA256 and
  compares that username and encrypted password to the usernames and passwords given in the config
  file specified in main.
  ...
  Also in this github repository is a c file named "encrypt_password.c" that can be executed in order to obtain the SHA256 hash for the password you enter. That way, if a different password is desired than the preset ones in config, it can be run through that c file and the output can be entered into the config for use with credentials.c.

  This program must be compiled with "-lssl -ljson-c -lcrypto"
*/
